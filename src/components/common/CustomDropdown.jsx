    ...
    return (
        <div className={`relative ${fullWidth ? 'w-full' : ''} ${className}`} ref={ref}>
            <button
                ...
                disabled={disabled}
                onClick={() => setOpen(o => !o)}
                className={`flex items-center justify-between gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm border transition-all ${
                ...
                <div className={`absolute top-full mt-2 z-50 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 max-h-[70vh] flex flex-col ${
                    ...
                    {allOption && (
                        <button
                            ...
                            className={`w-full flex items-center justify-between px-4 py-2.5 text-sm font-semibold border-b border-slate-100 transition-colors ${
                                String(value) === String(allOption.value) ? 'bg-[#2278B0]/5 text-[#2278B0]' : 'text-slate-500 hover:bg-slate-50'
                            }`}
                        >
