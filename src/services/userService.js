...
const calculatePerformanceMetrics = (questions, userAnswers, currentPerf) => {
    ...
    questions.forEach(q => {
        ...
        if (!perf.questionTypes[qType]) {
            const qText = (q.text || '').toLowerCase();
            if (qText.includes('how many of the above') || qText.includes('how many of the given') || (qText.includes('only one') && qText.includes('only two'))) qType = 'Statement (How many)';
            ...
            else if (qText.includes('match the following') || qText.includes('correctly matched') || qText.includes('pair 1') || qText.includes('pair 2') || qText.includes('match the pair') || qText.includes('match list')) qType = 'Match the pairs';
            else if ((qText.includes('assertion') && qText.includes('reason')) || (qText.includes('statement-i') && qText.includes('statement-ii')) || (qText.includes('statement i') && qText.includes('statement ii'))) qType = 'Assertion-Reason';
            else qType = 'One-liner';
        }

        if (!perf.questionTypes[qType]) {
            perf.questionTypes[qType] = { total: 0, attempted: 0, correct: 0 };
        }
